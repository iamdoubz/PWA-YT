...
def _finish(
    job_id: str, item_id: str, source_key: str, result: dict, scratch_dir: Path, user_id: str
) -> None:
    ...
    with db.writing() as conn:
        ...
        if result.get("title") or result.get("duration_s") is not None:
            ...
            if changed:
                conn.execute(
                    "UPDATE library_items SET updated_at=? WHERE id=?", (db.now(), item_id)
                )
...
def _runner() -> None:
    while not _stop.is_set():
        ...
        try:
            ...
            _finish(
                job["id"], job["item_id"], row["source_key"],
                future.result(timeout=0), scratch_dir, job["user_id"],
            )
